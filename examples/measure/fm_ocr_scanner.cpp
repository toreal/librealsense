...
std::tuple<bool, std::vector<cv::Point>, std::vector<cv::Mat> > ProcessEdgeImage( cv::Mat color_image, bool draw_debug_image) {
	
	cv::Mat edge_image;
	cv::cvtColor(color_image, edge_image, cv::COLOR_BGR2GRAY);

	cv::blur(edge_image, edge_image, cv::Size(3, 3));

	adaptiveThreshold(edge_image, edge_image, 255, cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY, 75, 10);
	//cv::bitwise_not(edge_image, edge_image);

	cv::Canny(edge_image, edge_image, 100, 100, 3);
